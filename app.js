...
(function () {
  ...
  // Source dates are dd-mm-yyyy; display them as "May 1st 2026".
  function formatDisplayDate(date) {
    const match = /^(\d{2})-(\d{2})-(\d{4})$/.exec((date || '').toString().trim());
    if (!match) return date || '';
    const day = parseInt(match[1], 10);
    const month = parseInt(match[2], 10);
    if (!day || day > 31 || !month || month > 12) return date;
    return `${MONTH_NAMES[month - 1]} ${ordinal(day)} ${match[3]}`;
  }
  ...
  function renderFilmEntry(film, idx, subsectionProviders) {
    ...
    const versionsHtml = versions.map((version) => {
      ...
      const versionSearch = [versionLanguage, title, date, formatDisplayDate(date)].join(' ').toLowerCase();
    ...
    return `<div class="entry" data-search="${escapeHtml(searchText)}" data-entry-meta="${escapeHtml(entryMeta)}" data-languages="${escapeHtml(uniqueLanguageKeys.join(' '))}">
      ${art ? `<img class="entry-art" loading="lazy" alt="" src="${escapeHtml(art)}">` : ''}
